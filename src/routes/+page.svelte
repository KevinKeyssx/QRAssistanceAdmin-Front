<script lang="ts">
	import { goto }     from '$app/navigation';
	import { onMount }  from 'svelte';
	import { env }      from '$env/dynamic/public';

    import { authClient }   from '$lib/auth/auth-client';
    import QRIcon           from '$lib/icons/QRIcon.svelte';


    onMount( () => {
		const checkMobile   = env.PUBLIC_CHECK_MOBILE === 'true';
		const isMobile      = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test( navigator.userAgent );

        if ( checkMobile && !isMobile ) goto( '/unauthorized' );
	});


    async function login() {
        await authClient.signIn.social({
            provider    : 'google',
            callbackURL : '/dashboard'
        });
    }
</script>


<svelte:head>
	<title>Asistencia QR · QRAsistenciaAdmin</title>
	<meta name="robots" content="noindex" />
</svelte:head>

<main class="flex flex-col items-center justify-center">
	<div class="animate-fade-in text-center">
		<div class="mb-8 flex flex-col items-center space-y-5">
            <div class="flex items-center gap-3">
                <div class="w-14 h-14 rounded-full bg-lds-navy dark:bg-lds-gold flex items-center justify-center shadow-lg text-white">
                    <QRIcon size={30}/>
                </div>

                <h1 class="text-2xl font-bold text-lds-navy dark:text-lds-gold">
                    Administrador Asistencia QR
                </h1>
            </div>

            <img src="/logo.avif" alt="Logo" class="fit-content size-40">

            <p class="text-sm text-gray-500 dark:text-gray-400">
				La Iglesia de Jesucristo de los Santos de los Últimos Días
			</p>

            <p class="text-gray-600 dark:text-gray-400 text-sm leading-relaxed max-w-xs">
                Accede al panel de administración para gestionar miembros, asistencias y visualizar estadísticas del sistema.
            </p>

            <div class="pt-6">
                <button
                    onclick = { login }
                    class   = "flex items-center gap-3 px-8 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full shadow-md hover:shadow-lg transition-all active:scale-95 group font-bold text-gray-700 dark:text-gray-200"
                >
                    <svg class="w-5 h-5" viewBox="0 0 48 48">
                        <path fill="#FFC107" d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12c0-6.627,5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24c0,11.045,8.955,20,20,20c11.045,0,20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z"></path>
                        <path fill="#FF3D00" d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"></path>
                        <path fill="#4CAF50" d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36c-5.202,0-9.619-3.317-11.283-7.946l-6.522,5.025C9.505,39.556,16.227,44,24,44z"></path>
                        <path fill="#1976D2" d="M43.611,20.083L43.595,20L24,20v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571c0.001-0.001,0.002-0.001,0.003-0.002l6.19,5.238C36.971,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z"></path>
                    </svg>
                    Iniciar Sesión con Google
                </button>
            </div>
        </div>
	</div>
</main>
