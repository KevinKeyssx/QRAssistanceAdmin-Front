<script lang="ts">
    ...
    let stars = $state<number>( 0 );

    onMount(async () => {
        try {
            const response = await fetch( 'https://github.com/KevinKeyssx/QRAssistance-Front' );
...
<button
    ...
    onclick = {() => window.open( "https://github.com/KevinKeyssx/QRAssistance-Front", "_blank" )}
>
    <span
    ...
    <div class="flex items-center gap-1">
        ...
        <span class="text-white">KevinKeyssx</span>

        <svg
            class="w-4 h-4 text-gray-500 transition-all duration-300 group-hover:text-amber-500"
            data-slot="icon"
            aria-hidden="true"
            fill="currentColor"
            viewBox="0 0 24 24"
        ...
        <!-- <span class="mt-[0.10rem] text-white"> -->
        <span class=" ">
            {stars}
        </span>
    </div>
